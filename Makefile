...
# Build flags
VPREFIX := github.com/pao214/loki/v3/pkg/util/build
...
test-fuzz:
	$(GOTEST) -timeout 30s -tags dev,gofuzz -cpuprofile cpu.prof -memprofile mem.prof  \
	  -run ^Test_Fuzz$$ github.com/pao214/loki/v3/pkg/logql -v -count=1 -timeout=0s

format:
	find . $(DONT_FIND) -name '*.pb.go' -prune -o -name '*.y.go' -prune -o -name '*.rl.go' -prune -o \
		-type f -name '*.go' -exec gofmt -w -s {} \;
	find . $(DONT_FIND) -name '*.pb.go' -prune -o -name '*.y.go' -prune -o -name '*.rl.go' -prune -o \
		-type f -name '*.go' -exec goimports -w -local github.com/pao214/loki/v3 {} \;
